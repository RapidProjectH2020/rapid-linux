...
public class NQueens extends Remoteable {
  ...
  private final static Logger log = LogManager.getLogger(NQueens.class.getSimpleName());
  ...
  private int localSolveNQueens(int N) {
    ...
    log.info("Finding solutions for " + N + "-queens puzzle.");
    log.info("Analyzing columns: " + start + "-" + (end - 1));
    ...
    log.info("Found " + countSolutions + " solutions.");
  ...
  public int localSolveNQueensReduce(int[] params) {
    ...
    for (int i = 0; i < params.length; i++) {
      log.info("Adding " + params[i] + " partial solutions.");
      solutions += params[i];
    }
    return solutions;
  }
