...
public class CudaDrFrontend {
    ...
    public String cuModuleLoadDataEx(String ptxSource, int jitNumOptions,
        ...
        System.out.println("cuModuleLoadDataEx 1");

        Buffer.clear();
        Buffer.AddInt(jitNumOptions);
        Buffer.Add(jitOptions);

        System.out.println("cuModuleLoadDataEx 2");

        // addStringForArgument
        ptxSource = ptxSource + "\0";
        long sizePtxSource = ptxSource.length();

        System.out.println("cuModuleLoadDataEx 2.1");

        long size = sizePtxSource;
        byte[] bits = Util.longToByteArray(size);

        System.out.println("cuModuleLoadDataEx 2.2");

        for (int i = 0; i < bits.length; i++) {
            Buffer.AddByte(bits[i] & 0xFF);
        }

        System.out.println("cuModuleLoadDataEx 2.3");

        for (int i = 0; i < bits.length; i++) {
            Buffer.AddByte(bits[i] & 0xFF);
        }

        System.out.println("cuModuleLoadDataEx 3");
        ...
        System.out.println("cuModuleLoadDataEx 4");
        ...
        System.out.println("cuModuleLoadDataEx 5");
        ...
        System.out.println("cuModuleLoadDataEx 6");

        int exit_c = Execute("cuModuleLoadDataEx");

        System.out.println("cuModuleLoadDataEx 6.1");

        Util.ExitCode.setExit_code(exit_c);

        System.out.println("cuModuleLoadDataEx 6.2");

        String pointer = "";
        pointer = Frontend.Transmitter.getHex(8);

        System.out.println("cuModuleLoadDataEx 7");

        for (int i = 0; i < Frontend.resultBufferSize - 8; i++)
            Frontend.in.readByte();

        System.out.println("cuModuleLoadDataEx 8");
